...
Meteor.methods({
  'consult_part_votes.insert'(consult_part_id){
    if(!this.userId){
      throw new Meteor.Error('403', "Vous devez vous connecter")
    }else{
      const vote = ConsultPartVotes.findOne({user: this.userId, consult_part: consult_part_id})
      if(vote){
        throw new Meteor.Error('403', "Vous avez déjà voté")
      }else{
        const new_vote = {user: this.userId, consult_part: consult_part_id}
